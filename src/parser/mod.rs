...
impl Parser {
    ...
    /// Parses and advances the token iterator until the optionally specified token type
    /// encountered; if encountered, the terminating token type is also consumned. If a terminating
    /// token is not specified, parsing continues until the iterator is empty.
